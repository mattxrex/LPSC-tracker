"""
Heartbeat / failure notification for LPSC Alerts.
...
The alerts tool once hung on a network call and quietly stopped sending alerts
for months. With this in place, that turns into an email instead of silence.
"""
...
def warn_if_stale():
    ...
    if days >= STALE_AFTER_DAYS:
        _safe_admin_alert(
            f"[LPSC Alerts] No successful check in {days} days",
            f"The last successful alerts check was {last:%Y-%m-%d %H:%M}.\n"
...
def notify_error(context, exc):
    ...
    _safe_admin_alert(
        f"[LPSC Alerts] Check failed: {context}",
